...
def run_beam_scot(task: str, beam_width: int = 4) -> int:
    # Provider selection: Groq primary, Ollama fallback
    if GROQ_API_KEY:
        ...
        print(f"[BEAM-SCOT] GROQ_API_KEY not set — falling back to Ollama ({OLLAMA_MODEL})", file=sys.stderr)
        client       = OpenAI(api_key="ollama", base_url=OLLAMA_BASE_URL)
        model        = OLLAMA_MODEL
        use_json_fmt = False   # gemma3:4b may not support json_object mode
